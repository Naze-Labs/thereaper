  ...
  async UpdateCard(req, res) {
    ...
    if (error) res.status(400).json({ msg: error.details[0].message });
      ...
      try {
        ...
        card.addressLine1 = (await addressLine1) || card.addressLine1;
        card.addressLine2 = (await addressLine2) || card.addressLine2;
        card.phone = (await phone) || card.phone;
        card.firstName = (await firstName) || card.firstName;
        card.lastName = (await lastName) || card.lastName;
        card.country = (await country) || card.country;
        card.state = (await state) || card.state;
        card.city = (await city) || card.city;
        card.postalCode = (await postalCode) || card.postalCode;
